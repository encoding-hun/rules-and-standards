...
## 6) Felbontás
  ...
  - 6.9) Javítás után a widow lineok (eredetileg fekete sávok) 1080p esetén eltávolíthatóak (resize vagy overlay), 720p esetén kötelezően eltávolítandóak.
...
## 7) Filterek
  ...
  - 7.2) Resize-oláshoz `z_Spline36Resize` vagy `Spline36ResizeMod` ajánlott, a `Spline36Resize` tartalmaz egy apró chroma shifting bugot, használata kerülendő. (VapourSynth-et nem érinti.) További engedett resizerek: `z_Spline64Resize` (`resize.Spline64`), `BlackmanResize`.
     - 7.2.1) Tilos `Nearest Neighbor`, `Bilinear`, `Bicubic`, és egyéb gyenge resizer használata.
...
## 8) Videó kódolás
  ...
  - 8.16) ColorMatrixot, amennyiben a forrás tartalmaz erre vonatkozó információt KÖTELEZŐ flaggelni (tipikusan `BT.709`), amennyiben nem, úgy `undef`-en kell hagyni.
  ...
  - 8.18) A maximálisan megengedett referencia képek számának használata kötelező (--ref).
    - 8.18.1) `--preset veryslow`/`placebo` magától kiszámolja a legnagyobbat, ami még nem töri meg a kompatibilitást. (Érdemes így csinálni, és akkor nem kell manuálisan számolni.)
    - 8.18.2) Kiszámolása: `8388608/(végső szélesség*végső magasság)` -> lefele kerekítés.
